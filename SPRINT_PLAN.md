...
```
Phase 1 ─── 安全加固 + 自动化采集 ─── ████████████ 100% ✅
      |
Phase 2 ─── 功能补全 ─── PDF 导出 + 搜索高亮 ████████████ 50%（PDF ✅ / 剩搜索高亮）
      |
Phase 3 ─── 设置页面 ─── 夜间模式 + 字号 + 语言偏好 ████████████ 100% ✅
      |
Phase 4 ─── 个人中心改版 ─── 统计 + 热力图 + 阅读习惯 + 账号管理 ⬜ 0%
```
...
### Phase 1
- [x] 替换 anon key，登录/注册/收藏/历史功能正常
- [x] RLS 策略已配置（bookmarks / reading_history / article_feedback / user_profiles）
- [x] 收藏按钮改为 SVG 五角星，移至文章信息区
- [x] 日报改为混合模型（按采集日期分组，每篇文章标注原始发布日期）
- [x] 历史日报数据已重建
- [x] GitHub OAuth 登录正常（登录页已有 GitHub 按钮）
- [x] Actions 定时采集正常（北京时间 6:00 / 12:00 / 18:00 自动运行）

### Phase 2
- [x] PDF 导出正常（html2canvas+jsPDF，中文完美+自动分页，直接下载）
- [x] 部署脚本 deploy.sh + GitHub Actions 自动部署 + 部署指南
- [ ] 搜索关键词高亮显示

### Phase 3
- [x] 浅色/深色/跟随系统三种模式正常切换（侧栏新增「设置」入口）
- [x] 字号三档切换有效（13px / 15px / 17px）
- [x] 语言偏好切换有效（中文/英文/全部，仅影响首页）
- [x] 所有偏好刷新后保持（localStorage 持久化）
