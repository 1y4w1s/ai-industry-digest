  ...
  async testAuth() {
    ...
    const tests = [
      ...
      },
      {
        name: '检查 supabase 配置',
        run: () => {
          const supabaseJs = fs.readFileSync(path.join(__dirname, 'src', 'lib', 'supabase.js'), 'utf-8');
          const hasGetSession = supabaseJs.includes('getSession');
          return { pass: hasGetSession, msg: hasGetSession ? '使用 getSession() (SDK v2)' : '可能使用了废弃的 session()' };
        }
      },
      {
