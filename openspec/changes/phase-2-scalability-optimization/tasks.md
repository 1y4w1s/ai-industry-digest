...
- [x] 1.2 添加 Python 依赖到 requirements.txt
  ```
  redis>=5.0.0
  pytest>=8.0.0
...
- [x] 2.1 创建缓存服务模块 `api/services/cache.py`
  - 实现 `CacheService` 类
  - 实现 `get`, `set`, `delete`, `get_stats` 方法
  - 实现缓存降级逻辑（Redis 不可用时直查数据库）

- [x] 2.2 修改 `api/models/database.py` 集成缓存
...
- [x] 3.2 编写 JWT 验证测试 `tests/test_jwt_verify.py`
  - 测试有效 token
  - 测试无效 token
  - 测试过期 token
  - 测试 demo 用户

- [x] 3.3 编写数据库测试 `tests/test_database.py`
  - 测试 `save_articles`
  - 测试 `get_articles` 分页
  - 测试 `get_articles` 搜索

- [x] 3.4 编写 API 测试 `tests/test_api.py`
  - 测试 `/api/articles` 端点
...
- [x] 4.1 创建 GitHub Actions 目录
  ```bash
  mkdir -p .github/workflows
  ```

- [x] 4.2 创建 CI 流程 `.github/workflows/ci.yml`
  - Python 环境配置
  - 安装依赖
  - 运行 pytest
  - 运行 ESLint（前端）

- [x] 4.3 创建 CD 流程 `.github/workflows/deploy.yml`
  - 触发条件：master 分支推送
  - 部署步骤：
    - SSH 到服务器
    - git pull
...
- [x] 4.5 添加回滚脚本 `scripts/rollback.sh`
  - 回滚到上一个版本
  - 重启服务
  - 验证健康状态
